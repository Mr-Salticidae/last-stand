...
![LAST STAND](https://raw.githubusercontent.com/Mr-Salticidae/last-stand/main/banner.png)

![主菜单](https://raw.githubusercontent.com/Mr-Salticidae/last-stand/main/assets/screenshots/%E4%B8%8A%E4%BC%A0itch/01_menu.png)

![战斗](https://raw.githubusercontent.com/Mr-Salticidae/last-stand/main/assets/screenshots/combat_v0.7.png)

![升级三选一](https://raw.githubusercontent.com/Mr-Salticidae/last-stand/main/assets/screenshots/%E4%B8%8A%E4%BC%A0itch/04_update.png)
...
![Godot 编辑器工作界面](https://raw.githubusercontent.com/Mr-Salticidae/last-stand/main/assets/screenshots/godot%E5%B1%95%E7%A4%BA.png)
...
适用场景：写系统骨架、批量重构、修 bug；**不适合替你判断手感——手感必须自己上手玩**。
...
![Autoload 全局单例面板](https://raw.githubusercontent.com/Mr-Salticidae/last-stand/main/assets/screenshots/tutorial/s1_autoload.png)

判断标准很简单：**这个东西需要跨场景活着吗？** 需要就 autoload，不需要就挂在场景里。玩家、敌人、武器都不是 autoload。
...
![文件系统目录结构](https://raw.githubusercontent.com/Mr-Salticidae/last-stand/main/assets/screenshots/tutorial/s2_filesystem.png)

### 2. 做核心循环：一个信号驱动的波次管理器

FPS 生存游戏的骨架就是"刷怪 → 打光 → 升级 → 再刷"。把它做成一个**只发信号、不直接操作 UI** 的管理器，后面接什么 UI 都不用改它。

见 `scripts/wave_manager.gd`，它对外只发这些信号：
...
![检查器中的 @export 分组](https://raw.githubusercontent.com/Mr-Salticidae/last-stand/main/assets/screenshots/tutorial/s3_inspector_export.png)
...
丢图即生效的目录约定（详见 `docs/enemy_sprite_animation.md`）：
...
加载逻辑在 `scripts/enemy.gd` 里按 `walk_0, walk_1, …` 顺序探测到缺失为止；**没有帧文件夹时自动退回单张静态图**，不报错也不需要改代码。所以可以先用单图把整个游戏跑通，之后想给哪只怪加动画就单独补它的帧文件夹——截至 v0.7.0，仓库里就还是 4 张单图，帧动画按需再画。
...
![导出对话框](https://raw.githubusercontent.com/Mr-Salticidae/last-stand/main/assets/screenshots/tutorial/s4_export_dialog.png)
...
![GitHub Release 页面](https://raw.githubusercontent.com/Mr-Salticidae/last-stand/main/assets/screenshots/tutorial/s5_github_release.png)

同一批内容再发一份到 itch.io，这边面向玩家、带评论区：

![itch.io 上架页](https://raw.githubusercontent.com/Mr-Salticidae/last-stand/main/assets/screenshots/itch%E4%B8%8A%E6%9E%B6%E9%A6%96%E9%A1%B5.png)
...
![hitbox 运行时可视化](https://raw.githubusercontent.com/Mr-Salticidae/last-stand/main/assets/screenshots/%E7%A2%B0%E6%92%9E.png)
...
**教训：每一处临时改动，提交信息里就打上 `debug` 标记**，发布前用 `git log --grep=debug` 扫一遍。
...
- **所用工具**：Godot 4.7（引擎）/ Claude Code（代码）/ Midjourney + GPT Image 2（精灵）/ Suno + ElevenLabs（音频）/ Quaternius + r2detta（模型）/ GitHub + itch.io（发布）
- **操作方法（六步顺序）**：① 立骨架（autoload 定全局状态）→ ② 核心循环（信号驱动波次管理器）→ ③ 填内容（解锁时间表造节奏）→ ④ 美术工业化（3D → 2D 单面精灵）→ ⑤ 版本节奏（每版一个主题）→ ⑥ 发布（导出 → Release → 三平台文案）。发布后的玩家反馈回流到第 ③ 步"填内容"，形成闭环。

**相关文档**：[敌人精灵逐帧动画出图规范](https://github.com/Mr-Salticidae/last-stand/blob/main/docs/enemy_sprite_animation.md) · [资产授权 CREDITS](https://github.com/Mr-Salticidae/last-stand/blob/main/CREDITS.md)
