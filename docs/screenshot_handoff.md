...
## 追加任务（2026-07-23）：S5 需重拍

> 首轮 S1–S5 已由桌面版补齐并推送（见回执）。此后主会话补传了构建产物，S5 的取景内容随之过时，需重拍**这一张**。其余 S1–S4 不动。

**背景**：拍 S5 时，v0.7.0 Release 页还没有构建附件，只有 GitHub 自动生成的源码归档。之后主会话已把原始构建 `LastStand-v0.7.0-windows.zip`（88.9MB）补传到该 Release。现在 Assets 区多了这个 zip，教程正文「打包产物传 GitHub Release」才真正对得上——但 S5 截图停在旧状态，要重拍才能体现。

**要做的**：

- **文件名**：`s5_github_release.png`（**覆盖同名文件**，不要改名）
- **操作路径**：浏览器打开 https://github.com/Mr-Salticidae/last-stand/releases/tag/v0.7.0
- **必须可见**：tag `v0.7.0` + 更新日志正文 + **Assets 区里的 `LastStand-v0.7.0-windows.zip`**（这是这次重拍的核心——旧图没有它，新图必须有）
- **构图 / 隐私**：同第 3 节与 S5 原规格（无书签栏、无其他标签页、无第三方窗口）
- 页面较长，务必滚到能同时看见更新日志和 Assets 附件区

**不需要做的**：

- 教程正文 [zero_to_release.md](zero_to_release.md) 里 S5 的引用路径没变，**回填无需改动**，覆盖图片文件即可
- 不要动 S1–S4，不要 commit/push（交回用户决定）

---

## 追加任务（2026-07-23）：S6 补拍成品展示的战斗画面

> 教程「一、成品展示」原有的战斗截图 `上传itch/02_combat.png` 是早期版本画面，已过时，由跳蛛先生手动删除。需补一张当前 v0.7 的战斗画面。

**这是成品展示图，不是操作过程图 —— 要进游戏实拍一局的战斗画面，不是编辑器界面。**

- **文件名**：`combat_v0.7.png`
- **存放**：`assets/screenshots/`（与其他成品图同级，**不进 tutorial/ 子目录**）
- **怎么拿**：用 `d:\Godot\Godot_v4.7.1-stable_win64.exe --path e:\last-stand` 打开项目 F5 运行，或直接跑已发布的构建；进任意一张地图打一波，截一张能体现当前观感的战斗画面
- **必须体现 v0.7 现状**：敌人是 **2D 单面精灵**（不是早期的 3D 机甲模型）；画面里能看到 HUD（波次 / 血量 / 武器栏）
- **构图 / 隐私**：横屏游戏画面，无第三方窗口
- **回填**：拍好后把教程 [zero_to_release.md](zero_to_release.md) 里 `> 📷 待补拍 S6` 那个占位块整块替换成 `![战斗](../assets/screenshots/combat_v0.7.png)`

---

## 经验：接手方编辑器的 format-on-save（2026-07-23 复盘）

首轮回填后发现 `zero_to_release.md` 的表格被整体重排成对齐格式（compact `|---|` → 带 padding 的对齐款）。这是接手方编辑器 "format on save" 对**整个文件**生效的结果，非有意改动。GitHub 渲染两种写法一致，本例无害，但换个规则就可能动到不该动的地方。

**下次派编辑器类接手方做回填/覆盖图时，二选一**：

1. **推荐**：接手方只负责拍图存文件，**回填与提交交回主会话（Claude Code）用精确 Edit 完成** —— 不在带 formatter 的编辑器里打开正文。
2. 若接手方必须动正文：先关闭 format-on-save，提交前用 `git diff` 自查，确认没有意外的全文重排 / 删行。

> 注：本次战斗图的删除**不是** formatter 干的，是跳蛛先生主动删的过时图（见上一节 S6）—— 别把这两件事混为一谈。
