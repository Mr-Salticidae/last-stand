# LastStand — Asset Credits

> Last updated: 2026-04-29 (demo release)

## Engine

- **Godot Engine 4.6** (MIT) — https://godotengine.org

## Music (BGM)

- **"Bunker Watch"** — generated with **Suno (Pro subscription)**, commercial use rights granted per Suno ToS
- File: `assets/audio/bgm/bunker_watch.mp3`

## Sound Effects (SFX)

- All 17 in-game SFX (gunshots, hits, kills, enemy windup, UI clicks, death audio, etc.) generated with **ElevenLabs (Pro subscription)**, commercial use rights granted per ElevenLabs ToS
- Files: `assets/audio/sfx/*.mp3`

## 3D Models

### Quaternius (CC0 - Public Domain)
- **Ultimate Gun Pack** — Pistol_3 / AssaultRifle_3 / Revolver_5 / Shotgun_2 / Bullpup_1
- **Sci-Fi Essentials Kit** — QuadShell / Trilobite / EyeDrone (enemy meshes)
- License: CC0 1.0 Universal — no attribution required (listed here as good practice)
- Source: https://quaternius.itch.io/
- Author: Quaternius (https://www.patreon.com/quaternius)

### r2detta (CC-BY 4.0 — **REQUIRED ATTRIBUTION**)
- **Low-Poly Weapon Asset Pack** — Minigun (heavy_mg legendary weapon)
- License: Creative Commons Attribution 4.0
- Source: https://sketchfab.com/3d-models/low-poly-weapon-asset-pack-762c43cc1532421eb0452b64e2bdd483
- Author: r2detta (https://sketchfab.com/r2detta)
- **License terms**: Free use including commercial, but author must be credited

### Animated Mech Pack (CC0 - Public Domain)
- **Mike / George** — used as Grunt / Boss enemies (Leela / Stan packed but excluded from build)
- License: CC0 1.0 Universal
- Listed here as good practice

## Fonts

- **Oswald** (variable) — Open Font License — https://fonts.google.com/specimen/Oswald
- **JetBrains Mono** (variable) — Open Font License — https://www.jetbrains.com/lp/mono/
- **Noto Sans SC** (variable) — Open Font License — https://fonts.google.com/noto

## HDRI / Skybox

- **Kloofendal 48d Partly Cloudy Puresky** — Polyhaven (CC0) — https://polyhaven.com/

---

## Notes for release

- This file is linked from the in-game Credits page (主菜单 → CREDITS)
- r2detta attribution is **legally required** per CC-BY license; cannot ship without it
- Suno / ElevenLabs commercial rights are conditional on active subscription at time of generation; both were Pro tier during 2026-04
- When adding new packs, append to this file with license info
